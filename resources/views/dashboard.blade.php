            ...
            <div class="mb-6 grid gap-4 md:grid-cols-4">
                <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                    <div class="flex items-center justify-between">
                        ...
                        <span class="rounded-full bg-cyan-100 px-2 py-1 text-xs font-semibold text-cyan-700">Today</span>
                    </div>
                ...
                <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                    <div class="flex items-center justify-between">
                        ...
                        <span
                            class="rounded-full bg-amber-100 px-2 py-1 text-xs font-semibold text-amber-700">Live</span>
                    </div>
                ...
                <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                    <div class="flex items-center justify-between">
                        <p class="text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">Pendapatan Hari Ini
                        </p>
                        <span
                            class="rounded-full bg-emerald-100 px-2 py-1 text-xs font-semibold text-emerald-700">Income</span>
                    </div>
                    <p class="mt-4 text-3xl font-bold text-emerald-600">Rp
                        {{ number_format((int) $incomeToday, 0, ',', '.') }}</p>
                        ...
                        @forelse ($areas as $area)
                            <div class="rounded-xl border border-slate-200 bg-slate-50 p-4">
                                <div class="mb-2 flex items-center justify-between gap-3">
                                    ...
                                    <p class="text-sm font-medium text-slate-700">{{ $area['terisi'] }} /
                                        {{ $area['kapasitas'] }}</p>
                                </div>

                                <div class="h-2.5 w-full overflow-hidden rounded-full bg-slate-200">
                                    <div class="h-full rounded-full bg-gradient-to-r from-cyan-500 to-sky-500 transition-all duration-300"
                        ...
                        @forelse ($recentTransactions as $transaction)
                            <div class="rounded-xl border border-slate-200 bg-slate-50 p-3">
                                <div class="flex items-center justify-between gap-3">
                                    ...
                                    <span
                                        class="inline-flex rounded-full px-2 py-1 text-[10px] font-semibold {{ $transaction->status === 'keluar' ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700' }}">
                                        {{ $transaction->status === 'keluar' ? 'Keluar' : 'Masuk' }}
                                    </span>
                                </div>
                                <div class="mt-2 text-xs text-slate-500">
                                    <p>{{ $transaction->jenisPelanggan?->nama ?? 'Reguler' }} •
                                        {{ $transaction->areaParkir?->nama ?? '-' }}</p>
