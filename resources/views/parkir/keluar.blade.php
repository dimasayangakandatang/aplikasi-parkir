...
<x-app-layout>
        ...
        <div class="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
            @if (session('success'))
                <div class="mb-6 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-700">
                    ...
                    @if (session('exit_ticket_transaction_id'))
                        <button type="button"
                            data-open-ticket="{{ route('parkir.ticket.exit.download', ['transaction' => session('exit_ticket_transaction_id')]) }}"
                            class="mt-2 inline-block font-semibold underline text-emerald-700 hover:text-emerald-800">
                            Buka karcis PDF
                        </button>
                    @endif
                </div>
            ...
            <div class="mb-6 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                ...
                <form id="exit-form" method="POST" action="{{ route('parkir.keluar.preview') }}" class="space-y-4">
                    ...
                    <div>
                        <label for="plat_nomor" class="mb-1 block text-sm font-medium text-slate-700">Nomor Polisi /
                            Nomor Karcis</label>
                    ...
                    <label
                        ...
                        <input type="checkbox" name="karcis_hilang" value="1"
                            {{ old('karcis_hilang') ? 'checked' : '' }}
                    ...
                    <div class="flex justify-end">
                        <button type="submit" id="preview-exit-button"
            ...
            <div class="rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-600">
                ...
                <a href="{{ route('parkir.terparkir') }}" class="font-semibold text-cyan-600 underline">Data Kendaraan
                    Terparkir</a>.
            </div>
        </div>
    </div>
    ...
    <div id="exit-preview-modal" class="fixed inset-0 z-50 hidden items-center justify-center bg-slate-900/60 p-4">
        <div class="w-full max-w-lg overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl">
            <div class="flex items-center justify-between border-b border-slate-200 px-5 py-4">
                <div>
                    <p class="text-xs font-semibold uppercase tracking-wider text-cyan-600">Konfirmasi</p>
                    <h3 class="text-lg font-bold text-slate-800">Preview Kendaraan Keluar</h3>
                </div>
                <button type="button" id="close-exit-preview"
                    class="rounded-lg border border-slate-200 px-3 py-1.5 text-sm text-slate-600 hover:bg-slate-100">Tutup</button>
            </div>

            <div class="space-y-4 p-5">
                <div class="grid grid-cols-2 gap-3 text-sm">
                    <div>
                        <p class="text-slate-500">Nomor Polisi</p>
                        <p id="preview-plat" class="font-semibold text-slate-800">-</p>
                    </div>
                    <div>
                        <p class="text-slate-500">Nomor Karcis</p>
                        <p id="preview-ticket" class="font-semibold text-slate-800">-</p>
                    </div>
                    <div>
                        <p class="text-slate-500">Jenis Kendaraan</p>
                        <p id="preview-vehicle" class="font-semibold capitalize text-slate-800">-</p>
                    </div>
                    <div>
                        <p class="text-slate-500">Jenis Pelanggan</p>
                        <p id="preview-customer" class="font-semibold text-slate-800">-</p>
                    </div>
                    <div>
                        <p class="text-slate-500">Area Parkir</p>
                        <p id="preview-area" class="font-semibold text-slate-800">-</p>
                    </div>
                    <div>
                        <p class="text-slate-500">Durasi</p>
                        <p id="preview-duration" class="font-semibold text-slate-800">-</p>
                    </div>
                    <div>
                        <p class="text-slate-500">Waktu Masuk</p>
                        <p id="preview-entry-time" class="font-semibold text-slate-800">-</p>
                    </div>
                    <div>
                        <p class="text-slate-500">Waktu Keluar</p>
                        <p id="preview-exit-time" class="font-semibold text-slate-800">-</p>
                    </div>
                </div>
                ...
                <div class="flex items-center justify-between rounded-xl bg-cyan-50 px-4 py-4">
                    <span class="font-semibold text-cyan-900">Total pembayaran</span>
                    <span id="preview-total" class="text-xl font-bold text-cyan-700">Rp 0</span>
                </div>
            </div>

            <div class="flex justify-end gap-3 border-t border-slate-200 bg-slate-50 px-5 py-4">
                <button type="button" id="cancel-exit-preview"
                    class="rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-100">Batal</button>
                <button type="button" id="confirm-exit-button"
                    class="rounded-lg bg-cyan-600 px-4 py-2 text-sm font-semibold text-white hover:bg-cyan-500">Proses
                    &amp; Simpan</button>
            </div>
        </div>
    </div>
    ...
    @if (session('exit_ticket_transaction_id'))
        <div id="ticket-modal" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 p-4">
            <div
                class="flex h-[90vh] w-full max-w-4xl flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl">
                <div class="flex items-center justify-between border-b border-slate-200 px-4 py-3">
                    <h3 class="text-base font-semibold text-slate-800">Preview Karcis</h3>
                    <button type="button" id="close-ticket-modal"
                        class="rounded-lg border border-slate-200 px-2 py-1 text-sm text-slate-600 hover:bg-slate-100">Tutup</button>
                </div>
                <iframe
                    src="{{ route('parkir.ticket.exit.download', ['transaction' => session('exit_ticket_transaction_id')]) }}"
                    class="h-full w-full bg-white" title="Karcis PDF"></iframe>
            </div>
        </div>
    @endif
                ...
                try {
                    ...
                    if (!response.ok) {
                        throw new Error(Object.values(data.errors || {}).flat()[0] ||
                            'Rincian kendaraan tidak dapat dihitung.');
                    }
