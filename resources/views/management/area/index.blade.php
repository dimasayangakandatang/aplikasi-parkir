                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-slate-200 text-left text-sm text-slate-700">
                        ...
                        <tbody class="divide-y divide-slate-200">
                                ...
                                <tr class="hover:bg-slate-50">
                                    ...
                                    <td class="px-4 py-3 text-right">
                                        <div class="flex justify-end gap-2">
                                            ...
                                            <form action="{{ route('management.area.destroy', $area) }}" method="POST"
                            ...
                            @empty
                                <tr>
                                    <td colspan="6" class="px-4 py-8 text-center text-slate-500">Belum ada area
                                        parkir.</td>
                                </tr>
                            @endforelse
                        </tbody>
            ...
            <div class="relative w-full max-w-xl rounded-2xl bg-white p-6 shadow-2xl">
                <div class="mb-6 flex items-center justify-between">
                    <h3 class="text-lg font-semibold text-slate-800"
                        x-text="mode === 'create' ? 'Tambah Area' : 'Edit Area'"></h3>
                            ...
                            <select x-model="form.tarif_id" name="tarif_id"
                                ...
                                @foreach ($tarifs as $tarif)
                                    <option value="{{ $tarif->id }}">{{ ucfirst($tarif->jenis_kendaraan) }} - Rp
                                        {{ number_format($tarif->tarif_jam_pertama, 0, ',', '.') }} / Rp
                                        {{ number_format($tarif->tarif_jam_berikutnya, 0, ',', '.') }}</option>
                                @endforeach
                            </select>
                        </div>

                        <div class="md:col-span-2">
                            <label class="mb-1 block text-sm font-medium text-slate-700">Jenis Pengguna yang Bisa Masuk
                                Area Ini</label>
                            <div class="grid gap-2 rounded-xl border border-slate-200 bg-slate-50 p-3 sm:grid-cols-2">
                                @foreach ($jenisPelanggans as $jenis)
                                    <label
                                        ...
                                        <input type="checkbox" name="jenis_pelanggan_ids[]" value="{{ $jenis->id }}"
